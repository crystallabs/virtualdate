require "../src/virtualdate"

yaml = "vdates.yml"
vdates = VirtualDate::VirtualDateFile.load(File.read(yaml))

scheduler = VirtualDate::Scheduler.new(vdates)

instances = [] of VirtualDate::Scheduled
1.upto(10) do |day|
  from, to = Time.local(2023, 5, day, 0, 0, 0), Time.local(2023, 5, day, 23, 59, 59)

  instances = scheduler.build(from, to)

  puts "-------------------------------------"
  puts "Schedule for #{from.to_s("%F")} (#{from.day_of_week})"

  instances
    .sort_by(&.start)
    .each do |instance|
      puts "%-10s  %s – %s  (%s)" % {
        instance.start.to_s,
        instance.finish.to_s,
        "\t",
        instance.vdate.id,
        instance.vdate.flags.join(", "),
      }

      puts "\t#{instance.vdate.id} @ #{instance.start}"
      puts "\t" + instance.explanation.lines.join("\n\t")
    end
end

# For example, only the last displayed day is in ICS
ics = VirtualDate::ICS.export(
  instances,
  calendar_name: "My Task Schedule"
)
File.write("vdates.ics", ics)

file = {
  "schema_version" => VirtualDate::Migrator::CURRENT_VERSION,
  "vdates"         => vdates,
}

File.write("vdates.yml", file.to_yaml)
